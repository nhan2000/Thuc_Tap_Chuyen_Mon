...
class Admin extends MY_Controller {
	...
	public function add()
	{
        ...
        // nếu dữ liệu được post lên ta sẽ kiểm tra các rule cho các thể input
		if ($this->input->post()) {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {				
				...
				if ($this->admin_model->update($id,$data)) {
					$this->session->set_flashdata('message_success', 'Cập nhật tài khoản thành công');
				}else{
					$this->session->set_flashdata('message_fail', 'Cập nhật tài khoản thất bại');
				}
				redirect(admin_url('admin'));
			}
		}
