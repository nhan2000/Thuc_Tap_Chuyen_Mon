...
class Product extends MY_Controller {
	...
	public function index()
	{
		...
		$config = array();
		$base_url = admin_url('product/index');//link hiển thị ra dữ liệu
		$per = 10;
		$uri = 4;
		$config = pagination($base_url,$total,$per,$uri);
		$this->pagination->initialize($config);//phương thức khởi tạo cấu hình phân trang
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->product_model->update($id,$data)) {
					...
					$this->session->set_flashdata('message_fail', 'Cập nhật sản phẩm thất bại');
				}
				redirect(admin_url('product'));
			}
		}
